...
#include <string>
#include <ctime>
#include "utils.h" // new library header
...
void OUTFile(vector<Cart>&cart){ // customers outfile
    ...
    for(int i = 0; i < cart.size(); i++){
        cust << fixed << setprecision(2) << cart[i].type << " " << cart[i].qty << " " << cart[i].payment << " " << cart[i].notes << endl; // write the data to the file
    }
    cust.close();
}
...
double Calc(char op, vector<Cart>& cart){
    ...
    do{
        ...
        if(qty < 1){
            cout << "The quantity must not less than 1!" << endl;
        } else  if(qty > 10){
            cout << "Max order quantity is 10!" << endl;
        }
    } while(qty < 1 || qty > 10);
